...
def load_tokens(server_name):
    """Loads the token file based on the server name."""
    try:
        if server_name == "IND":
            path = "token_ind.json"
...
def get_url(server_name):
    """Returns the correct game server URL based on the server name."""
    if server_name == "IND":
...
def parse_protobuf_response(response_data):
    """Parses the protobuf response and extracts player data."""
    try:
        info = Info()
...
async def send_until_10000_success(tokens, uid, server_name, target_success=10000):
    """Sends requests until 10,000 successful visits are achieved."""
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        # Encrypt the UID and prepare the request data
        encrypted = encrypt_api("08" + Encrypt_ID(str(uid)) + "1801")
        data = bytes.fromhex(encrypted)

        while total_success < target_success:
            batch_size = min(target_success - total_success, 1000) # Adjust batch size to manage CPU usage
            tasks = [
            ...
            if first_success_response is None:
                for success, response in results:
                    if success and response is not None:
                        ...
                        break # Get player info from the first successful response
...
def send_visits(server, uid):
    """The main API endpoint for sending visits."""
    server = server.upper()
    ...
    print(f"Waiting for a total of {target_success} successful visits...")
    ...
    if player_info:
        ...
        return jsonify({"error": "Could not decode player information", "success_count": total_success}), 500
